<?php
    ...
    switch(@$proc){
        ...
        case "add" :
            $sql=$db->prepare("INSERT INTO user(username, password, nama, foto) VALUES (:username, :password, :nama, :foto)");
        ...
        case "edit" :
            $sql=$db->prepare("UPDATE user SET password = :password, nama = :nama, foto = :foto WHERE username = :username");
            $sql->bindParam(':username', $username);
            $sql->bindParam(':password', $password);
            $sql->bindParam(':nama', $nama);
            $sql->bindParam(':foto', $tujuan);
